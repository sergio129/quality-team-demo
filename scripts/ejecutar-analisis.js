// Script para ejecutar análisis de SonarQube con Docker Compose
...
try {
  ...
  // Leer el token del archivo de propiedades si no está en las variables de entorno
  let tokenFromFile = '';
  const propFile = path.join(process.cwd(), 'sonar-project.properties');
  if (fs.existsSync(propFile)) {
    const content = fs.readFileSync(propFile, 'utf8');
    const tokenMatch = content.match(/sonar\.login=([^\r\n]+)/);
    if (tokenMatch && tokenMatch[1]) {
      tokenFromFile = tokenMatch[1];
    }
  }
  
  const finalToken = sonarToken || tokenFromFile;
  
  const tokenMessage = finalToken 
