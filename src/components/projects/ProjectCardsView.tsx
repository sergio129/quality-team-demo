...
export default function ProjectCardsView({
  ...
  // Función para obtener los colores de la tarjeta según el estado
  const getCardColors = (project: Project) => {
    const estado = project.estado?.toLowerCase();
    
    // Priorizar estado "completado" (certificado) - siempre verde aunque esté retrasado
    ...
    // Si está retrasado y no está completado
    ...
    switch (estado) {
      // case para "completado" ya manejado arriba
      case 'en progreso':
        return {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {projects.map((project) => {
        const cardColors = getCardColors(project);
        
        return (
          <div
            key={project.id}
            className={`bg-white rounded-lg shadow-md border transition-shadow duration-200 ${cardColors.border} ${cardColors.shadow} ${cardColors.background}`}
          >
            ...
            </div>          {/* Footer con acciones */}          <div className={`px-4 py-3 border-t border-gray-100 rounded-b-lg ${
            project.estado?.toLowerCase() === 'completado' ? 'bg-green-50' :
            isProjectDelayed(project) ? 'bg-red-50' : 
            project.estado?.toLowerCase() === 'en progreso' ? 'bg-blue-50' :
            project.estado?.toLowerCase() === 'pendiente' ? 'bg-yellow-50' :
            'bg-gray-50'
          }`}>
